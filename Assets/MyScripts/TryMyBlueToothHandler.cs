...
public class TryMyBlueToothHandler : MonoBehaviour
{
	//TODO:
	//用IEnumerator来实现
    ...
    private IEnumerator StartProcess()
    {
        ...
        yield return StartCoroutine(CallScanCor());
        yield return StartCoroutine(CallConnectCor());
        yield return StartCoroutine(CallRequestMtuCor());
        yield return StartCoroutine(CallSubscribeCor());
    }
    ...
    private IEnumerator CallScanCor()
    {
    ...
    private IEnumerator CallConnectCor()
    {
        for (int i = 0; i < DeviceList.Count; i++)
        {
    ...
    private IEnumerator CallRequestMtuCor()
    {
        for (int i = 0; i < DeviceList.Count; i++)
        {
    ...
    private IEnumerator CallSubscribeCor()
    {
        for (int i = 0; i < DeviceList.Count; i++)
        {
           CallSubscribe(i);
    ...
    private void CallSubscribe(int _index)
    {
			...
			if(_crow != null){
			    if(DeviceList[_index].ControllerName == "OldBalanceBall"){
					if(GameManager.Instance.cooperationMode == 0)
					    ...
					    _crow.GetComponent<PlayerController>().ControlPlayerVerMovement(_q, 5); //因为平衡球幅度小, 所以range应该小
